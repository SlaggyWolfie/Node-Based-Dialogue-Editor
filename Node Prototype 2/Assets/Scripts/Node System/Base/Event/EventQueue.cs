...
namespace Wolfram.RPG.Events
{
    public class EventQueue
    {
        ...
        {
            get { return _instance ?? (_instance = new EventQueue()); }
        }

        private Queue<Event> _events = new Queue<Event>();

        public delegate void EventHandler<T>(T delegateEvent) where T : Event;
        ...
        private Dictionary<System.Delegate, EventDelegate> _handlersToDelegates =
            ...
            where T : Event
        {
            ...
            {
                Console.WriteLine(
                    "You cannot add the same method/event handler as a listener twice. Please use another way.");
                return;
            }
